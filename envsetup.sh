...
function setpaths()
{
    ...
    TARGET_ARM_EABI_TOOLCHAIN_DIR=$(get_build_var ARM_EABI_TOOLCHAIN_DIR)
    export ARM_EABI_TOOLCHAIN_DIR=$TARGET_ARM_EABI_TOOLCHAIN_DIR

    TARGET_ANDROID_EABI_TOOLCHAIN_DIR=$(get_build_var ANDROID_EABI_TOOLCHAIN_DIR)
    export ANDROID_EABI_TOOLCHAIN_DIR=$TARGET_ANDROID_EABI_TOOLCHAIN_DIR
    ...
    case $ARCH in
        x86) toolchaindir=x86/i686-linux-android-4.6/bin
            ;;
        arm) toolchaindir=arm/$ANDROID_EABI_TOOLCHAIN_DIR/bin
            ;;
    ...
    case $ARCH in
        arm)
            toolchaindir=arm/$ARM_EABI_TOOLCHAIN_DIR/bin
            if [ -d "$gccprebuiltdir/$toolchaindir" ]; then
